...
func newBackupOnlyCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "backup-only",
...
Uses pure Go PostgreSQL dump (no pg_dump binary required).`,
	...
	cmd.Flags().StringP("config", "c", "", "[required] Path to the backup YAML configuration file")
...
func runBackupOnly(cmd *cobra.Command, args []string) error {
	...
	config, err := models.LoadBackupConfig(configPath)
	if err != nil {
