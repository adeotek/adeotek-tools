...
# Integration test script for sql-migration

set -e

echo "Running integration tests for sql-migration..."
...
./build/sql-migration --target-path ./test-scripts --provider sqlite --database test-integration.db --verbose
...
OUTPUT=$(./build/sql-migration --target-path ./test-scripts --provider sqlite --database test-integration.db --verbose 2>&1)
...
OUTPUT=$(./build/sql-migration --target-path ./test-scripts --provider sqlite --database test-integration2.db --dry-run --verbose 2>&1)
...
OUTPUT=$(./build/sql-migration --version 2>&1)

if echo "$OUTPUT" | grep -q "sql-migration version"; then
